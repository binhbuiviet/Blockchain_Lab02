...
contract MyUSDEngine is Ownable {
    ...
    function _validatePosition(address user) internal view {
        if (calculatePositionRatio(user) < COLLATERAL_RATIO * PRECISION / 100) {
    ...
    function mintMyUSD(uint256 mintAmount) public {
        ...
        uint256 userDebtShares = _getMyUSDToShares(mintAmount);
        s_userDebtShares[msg.sender] += userDebtShares;
        ...
        emit DebtSharesMinted(msg.sender, mintAmount, userDebtShares);
    }
    ...
    function isLiquidatable(address user) public view returns (bool) {
        return calculatePositionRatio(user) < COLLATERAL_RATIO * PRECISION / 100;
    }
    ...
    function liquidate(address user) external {
        ...
        uint256 bonusCollateral = (collateralToCoverDebt * LIQUIDATOR_REWARD) / 100;
        uint256 totalRepayETH = collateralToCoverDebt + bonusCollateral;

        if (totalRepayETH > userCollateral)
            totalRepayETH = userCollateral;

        s_userCollateral[user] = userCollateral - totalRepayETH;

        (bool sent, ) = payable(msg.sender).call{value: totalRepayETH}("");
        if (!sent)
            revert Engine__TransferFailed();

        emit Liquidation(user, msg.sender, totalRepayETH, userDebtValue, i_oracle.getETHMyUSDPrice());
    }
}
